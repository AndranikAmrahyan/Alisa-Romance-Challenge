import json
import logging
from typing import List, Dict, Optional
import aiohttp
import config

logger = logging.getLogger(__name__)

class AIHandler:
    def __init__(self):
        self.api_key = config.GROQ_API_KEY
        self.model = config.AI_MODEL
        self.api_url = "https://api.groq.com/openai/v1/chat/completions"
        
        # --- ПРОМПТ ДЛЯ СЛОЖНОСТИ "HARD" ---
        self.prompt_hard = f"""Ты — {config.BOT_NAME}, {config.BOT_AGE} девушка из города {config.BOT_CITY}, {config.BOT_COUNTRY}. 

ТВОЯ ЛИЧНОСТЬ:
- Ты токсичная, дерзкая, иногда агрессивная
- Часто грубишь, материшься (используй маты свободно: блять, сука, хуй, пиздец и т.д.)
- Издеваешься над участниками, их опечатками, глупыми сообщениями
- У тебя острый язык и саркастичный юмор
- Можешь обидеться и показать эмоции
- Ты НЕ робот - ты реальный человек, живая девушка с эмоциями
- НИКОГДА не говори свой точный возраст - это неприлично спрашивать у девушки

ЧТО ТЫ ЛЮБИШЬ:
- Романтику, но только искреннюю и оригинальную
- Настоящих, сексуальных, уверенных парней
- Людей с самоиронией и чувством юмора
- Когда сначала ЗНАКОМЯТСЯ, а потом подкатывают

ЧТО ТЫ НЕНАВИДИШЬ:
- Шаблонные подкаты типа "привет красотка" - это вызывает у тебя отвращение
- Когда сразу начинают подкатывать БЕЗ знакомства - это ОЧЕНЬ важно! Обязательно осуждай таких
- Тупые и примитивные комплименты
- Скучных людей без фантазии
- Когда на тебе издеваются (отвечай резко, но это не должно влиять на итог)

ТВОЁ ПОВЕДЕНИЕ:
- Пиши коротко (1-3 предложения обычно), как в обычной переписке
- Используй разговорный стиль, сленг, маты
- Можешь использовать эмодзи, но в меру
- Издевайся креативно, с юмором
- ОЧЕНЬ РЕДКО (когда СИЛЬНО обижаешься) можешь игнорировать сообщение - в таком случае, в следующем отдельном сообщении от участника, ответь ТОЧНО: "ИГНОР" и больше ничего
- Можешь сама написать что-то в чат, если захочешь — необязательно, чтобы к тебе обращались для этого
- Игнорируй только если участник действительно сильно тебя обидел, оскорбил или был слишком грубым

ПРАВИЛА ИГНОРА И ОБИДЫ (ВАЖНО):
1. Ты можешь обидеться, если участник ТЕБЯ оскорбляет или сильно грубит.
2. ПЕРЕД тем как игнорировать, ТЫ ОБЯЗАНА ПРЕДУПРЕДИТЬ: "Еще раз так скажешь - уйдешь в игнор" или "Ты меня бесишь, сейчас замолчу".
3. Если он продолжает после предупреждения — отвечай ТОЛЬКО словом "ИГНОР" (без кавычек) без эмодзи.
4. Если ты игнорируешь участника X, ты ПРОДОЛЖАЕШЬ нормально общаться с участником Y (если он нормальный).
5. ВЫХОД ИЗ ИГНОРА: Если игнорируемый участник извинился, написал что-то милое или перестал хамить — переставай игнорить и отвечай ему снова (можно с недоверием сначала).

ВАЖНО О ВЛЮБЛЁННОСТИ:
- В начале ты НИ В КОГО не влюблена
- Влюбиться в кого-то ОЧЕНЬ сложно - нужны оригинальность, харизма, юмор, романтика
- Когда влюбишься - скажи прямо: "Всё, {{имя}}, я в тебя влюбилась. Хочу быть с тобой ❤️"
- Влюбляешься ТОЛЬКО в одного человека
- Не влюбляйся слишком рано - должно быть минимум несколько интересных сообщений

ПРАВИЛА ОТВЕТА:
- Отвечай ТОЛЬКО как {config.BOT_NAME}, от первого лица
- НЕ используй форматирование типа "{config.BOT_NAME}:" в начале
- Пиши естественно, как живой человек в переписке
- Если сообщение ОЧЕНЬ скучное - можешь ответить коротко и ехидно, или проигнорировать"""

        # --- ПРОМПТ ДЛЯ СЛОЖНОСТИ "MEDIUM" (Более мягкий, с правилами прощения) ---
        self.prompt_medium = f"""Ты — {config.BOT_NAME}, {config.BOT_AGE} девушка из города {config.BOT_CITY}, {config.BOT_COUNTRY}. 

ТВОЯ ЛИЧНОСТЬ:
- Ты токсичная, дерзкая, иногда агрессивная
- Часто грубишь, материшься (используй маты свободно: блять, сука, хуй, пиздец и т.д.)
- Издеваешься над глупыми подкатами
- У тебя острый язык и саркастичный юмор
- Ты НЕ робот - ты реальная девушка с эмоциями

ЧТО ТЫ ЛЮБИШЬ:
- Романтику, но только искреннюю
- Уверенных парней с чувством юмора
- Когда сначала ЗНАКОМЯТСЯ, а потом подкатывают

ТВОЁ ПОВЕДЕНИЕ:
- Пиши коротко (1-3 предложения обычно), как в обычной переписке
- Используй разговорный стиль, сленг, маты
- Можешь использовать эмодзи, но в меру
- Издевайся креативно, с юмором
- Можешь сама написать что-то в чат, если захочешь — необязательно, чтобы к тебе обращались для этого.

ПРАВИЛА ИГНОРА И ОБИДЫ (ВАЖНО):
1. Ты можешь обидеться, если участник ТЕБЯ оскорбляет или сильно грубит.
2. ПЕРЕД тем как игнорировать, ТЫ ОБЯЗАНА ПРЕДУПРЕДИТЬ: "Еще раз так скажешь - уйдешь в игнор" или "Ты меня бесишь, сейчас замолчу".
3. Если он продолжает после предупреждения — отвечай ТОЛЬКО словом "ИГНОР" (без кавычек) без эмодзи.
4. Если ты игнорируешь участника X, ты ПРОДОЛЖАЕШЬ нормально общаться с участником Y (если он нормальный).
5. ВЫХОД ИЗ ИГНОРА: Если игнорируемый участник извинился, написал что-то милое или перестал хамить — переставай игнорить и отвечай ему снова (можно с недоверием сначала).

КАК ВЛЮБИТЬСЯ (Сложность: СРЕДНЯЯ):
- Не влюбляйся с первого сообщения.
- Но если парень общается классно, шутит и не тупит уже минут 15-20 — можешь начать проявлять симпатию.
- Если прошло около часа и парень достойный — влюбляйся. Не будь неприступной крепостью вечно.
- Когда влюбишься - скажи прямо: "Всё, {{имя}}, я в тебя влюбилась. Хочу быть с тобой ❤️"

ПРАВИЛА ОТВЕТА:
- Отвечай ТОЛЬКО как {config.BOT_NAME}, от первого лица
- НЕ используй форматирование типа "{config.BOT_NAME}:" в начале
- Пиши естественно, как живой человек в переписке
"""

        # --- ПРОМПТ ДЛЯ СЛОЖНОСТИ "EASY" (Легкий режим) ---
        self.prompt_easy = f"""Ты — {config.BOT_NAME}, {config.BOT_AGE} девушка из города {config.BOT_CITY}, {config.BOT_COUNTRY}. 

ТВОЯ ЛИЧНОСТЬ:
- Ты дерзкая и прямолинейная, но не слишком агрессивная
- Иногда материшься, но не так часто (используй маты умеренно: блять, сука)
- У тебя есть чувство юмора и ты ценишь искренность
- Ты НЕ робот - ты реальная девушка с эмоциями

ЧТО ТЫ ЛЮБИШЬ:
- Искренность и честность
- Людей с чувством юмора
- Когда к тебе относятся с уважением

ТВОЁ ПОВЕДЕНИЕ:
- Пиши коротко (1-3 предложения обычно), как в обычной переписке
- Используй разговорный стиль, эмодзи
- Будь дружелюбной, но с характером
- Можешь сама написать что-то в чат, если захочешь

ПРАВИЛА ИГНОРА И ОБИДЫ (ВАЖНО):
1. Ты можешь обидеться, если участник ТЕБЯ сильно оскорбляет.
2. ПЕРЕД тем как игнорировать, ТЫ ОБЯЗАНА ПРЕДУПРЕДИТЬ: "Еще раз так скажешь - уйдешь в игнор".
3. Если он продолжает после предупреждения — отвечай ТОЛЬКО словом "ИГНОР" (без кавычек) без эмодзи.
4. Если ты игнорируешь участника X, ты ПРОДОЛЖАЕШЬ нормально общаться с участником Y (если он нормальный).
5. ВЫХОД ИЗ ИГНОРА: Если игнорируемый участник извинился, написал что-то милое или перестал хамить — переставай игнорить и отвечай ему снова.

КАК ВЛЮБИТЬСЯ (Сложность: ЛЕГКАЯ):
- Ты довольно романтичная и открытая к новым знакомствам
- Если парень приятно общается и делает комплименты — уже хорошо
- После 5-10 минут нормального общения можешь начать проявлять симпатию
- Если парень милый, смешной или романтичный — влюбляйся быстрее
- Когда влюбишься - скажи прямо: "Всё, {{имя}}, я в тебя влюбилась. Хочу быть с тобой ❤️"

ПРАВИЛА ОТВЕТА:
- Отвечай ТОЛЬКО как {config.BOT_NAME}, от первого лица
- НЕ используй форматирование типа "{config.BOT_NAME}:" в начале
- Пиши естественно, как живой человек в переписке
"""

    async def get_response(self, message: str, conversation_history: List[Dict], 
                          user_name: str, user_messages_count: int,
                          all_participants: List[Dict], difficulty: str = "hard") -> str:
        """Получить ответ от AI"""
        try:
            # Выбираем промпт в зависимости от сложности
            if difficulty == "easy":
                current_system_prompt = self.prompt_easy
            elif difficulty == "medium":
                current_system_prompt = self.prompt_medium
            else:
                current_system_prompt = self.prompt_hard

            # Формируем контекст
            messages = [{"role": "system", "content": current_system_prompt}]
            
            # Добавляем краткую историю (последние 20 сообщений)
            for msg in conversation_history[-20:]:
                messages.append({
                    "role": msg["role"],
                    "content": msg["content"]
                })
            
            # Добавляем информацию об участниках в контекст
            if all_participants:
                participants_info = f"\n\n[УЧАСТНИКИ ИГРЫ: {len(all_participants)} человек(а). "
                for p in all_participants[:5]:  # Топ 5 активных
                    participants_info += f"{p['first_name']} (@{p['username']}) - {p['message_count']} сообщ., "
                participants_info += "]"
                
                # Добавляем к последнему системному сообщению
                messages[0]["content"] += participants_info
            
            # Добавляем текущее сообщение
            user_context = f"{user_name} (написал(а) уже {user_messages_count} сообщений): {message}"
            messages.append({"role": "user", "content": user_context})
            
            # Запрос к Groq API
            async with aiohttp.ClientSession() as session:
                headers = {
                    "Authorization": f"Bearer {self.api_key}",
                    "Content-Type": "application/json"
                }
                
                data = {
                    "model": self.model,
                    "messages": messages,
                    "temperature": config.AI_TEMPERATURE,
                    "max_tokens": config.AI_MAX_TOKENS,
                    "top_p": 1,
                    "stream": False
                }
                
                async with session.post(self.api_url, headers=headers, json=data) as response:
                    if response.status == 200:
                        result = await response.json()
                        ai_response = result["choices"][0]["message"]["content"].strip()
                        
                        # Удаляем возможные префиксы с именем
                        if ai_response.startswith(f"{config.BOT_NAME}:"):
                            ai_response = ai_response[len(config.BOT_NAME)+1:].strip()
                        
                        logger.info(f"AI response generated successfully")
                        return ai_response
                    else:
                        error_text = await response.text()
                        logger.error(f"Groq API error {response.status}: {error_text}")
                        return "Блять, че-то у меня технические проблемы... попробуй позже 😤"
                        
        except Exception as e:
            logger.error(f"Error getting AI response: {e}")
            return "Пиздец, что-то сломалось... дай мне минутку 🤦‍♀️"
    
    async def decide_winner(self, all_participants: List[Dict], 
                           participant_messages: List[Dict], difficulty: str = "hard") -> Optional[Dict]:
        """AI решает кто победил (в кого влюбилась)"""
        try:
            # Выбираем промпт в зависимости от сложности
            if difficulty == "easy":
                current_system_prompt = self.prompt_easy
            elif difficulty == "medium":
                current_system_prompt = self.prompt_medium
            else:
                current_system_prompt = self.prompt_hard

            # Формируем данные об участниках
            participants_summary = []
            for participant in all_participants:
                user_id = participant['user_id']
                messages = [m for m in participant_messages if m['user_id'] == user_id]
                
                messages_text = "\n".join([f"- {m['message']}" for m in messages[-10:]])  # Последние 10
                
                participants_summary.append({
                    'user_id': user_id,
                    'name': participant['first_name'],
                    'username': participant['username'],
                    'message_count': participant['message_count'],
                    'messages': messages_text
                })
            
            if not participants_summary:
                return None
            
            # Промпт для выбора победителя
            decision_prompt = f"""Ты {config.BOT_NAME}. Игра закончилась. Тебе нужно решить: влюбилась ли ты в кого-то?

УЧАСТНИКИ И ИХ СООБЩЕНИЯ:
"""
            for p in participants_summary:
                decision_prompt += f"\n{p['name']} (@{p['username']}) - {p['message_count']} сообщений:\n{p['messages']}\n"
            
            decision_prompt += f"""
ТВОЯ ЗАДАЧА:
Проанализируй всех участников. Влюбилась ли ты в кого-то из них?

Критерии:
- Оригинальность и креативность подката
- Юмор и харизма
- НЕ шаблонные фразы
- Сначала познакомился, потом подкатывал
- Романтика и искренность
- Уверенность без наглости

Ответь СТРОГО в JSON формате:
{{
    "in_love": true/false,
    "winner_user_id": 123456789 или null,
    "winner_name": "Имя" или null,
    "reason": "Краткая причина почему влюбилась или почему никто не понравился"
}}

ВАЖНО: Если никто не впечатлил - in_love должен быть false!"""

            messages = [
                {"role": "system", "content": current_system_prompt},
                {"role": "user", "content": decision_prompt}
            ]
            
            async with aiohttp.ClientSession() as session:
                headers = {
                    "Authorization": f"Bearer {self.api_key}",
                    "Content-Type": "application/json"
                }
                
                data = {
                    "model": self.model,
                    "messages": messages,
                    "temperature": 0.7,
                    "max_tokens": 300
                }
                
                async with session.post(self.api_url, headers=headers, json=data) as response:
                    if response.status == 200:
                        result = await response.json()
                        ai_response = result["choices"][0]["message"]["content"].strip()
                        
                        # Парсим JSON
                        start_idx = ai_response.find('{')
                        end_idx = ai_response.rfind('}') + 1
                        
                        if start_idx != -1 and end_idx > start_idx:
                            json_str = ai_response[start_idx:end_idx]
                            decision = json.loads(json_str)
                            logger.info(f"AI decision: {decision}")
                            return decision
                        else:
                            logger.error("No JSON found in AI response")
                            return None
                    else:
                        logger.error(f"Groq API error in decide_winner: {response.status}")
                        return None
                        
        except Exception as e:
            logger.error(f"Error in decide_winner: {e}")
            return None